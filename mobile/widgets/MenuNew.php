...
class MenuNew extends Widget
{
    ...
    {    
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {                  
            ...
            if (ltrim($route, '/').'index' !== $this->route&&ltrim($route, '/') !== $this->route) {                           
                return false;
            }                    
            unset($item['url']['#']);
            if (count($item['url']) > 1) {
                ...
                foreach ($params as $name => $value) {
                    if(!isset($this->params[$name])) // 在act/page路径下，如果获取不到view，强制把view变成index3
                    {
                        $this->params[$name] = "index3";
                    }                    
        ...
        {   
            $thisurl = Yii::$app->request->hostInfo;
            $thisurl .= Yii::$app->request->getUrl();
            if ($item['url']==$thisurl) {
                return true;
            }       
        }
