...
<!--                                    <input type="text" class="text" name="shouHuoXinXi[addr]" value="" placeholder="请输入收货地址" required="required" data-caution="收获地址不能为空">-->
            <?= $form->field($model, 'stateName', ['labelOptions' => ['style' => 'display:none'],'errorOptions' => ['style' => 'display:none']])->textInput(['type' => 'hidden']) ?>
            <?= $form->field($model, 'cityName', ['labelOptions' => ['style' => 'display:none'],'errorOptions' => ['style' => 'display:none']])->textInput(['type' => 'hidden']) ?>
            <?= $form->field($model, 'districtName', ['labelOptions' => ['style' => 'display:none'],'errorOptions' => ['style' => 'display:none']])->textInput(['type' => 'hidden']) ?>
            <script>
                jQuery(function () { 
                ...
                //Message.error("加入购物车失败.<br /><ul><li>可能库存不足.</li><li>或提交信息不完整.</li></ul>");
                        ...
                        <?= $form->field($model, 'address',['labelOptions' => ['class' => 'c-l','style' =>'line-height:26px']])->textarea([ 'class' => 'text', 'rows' => 2, 'cols' => 30 ,  'style'=>'max-width:690px;height:52px;line-height:26px;max-height:340px']) ?>
                            ...
                            <div class="c-g">       
                                    ...
                                    <?= $form->field($model, 'receiverPhone',['labelOptions' => ['class' => 'c-l']])->textInput(['class' => 'text', 'size' => 15]) ?>
                            </div>
                            <div class="c-g-c">
...
<!--                        </form>-->
                ...
                <a href="<?= Url::to(['passport/registerfinish']) ?>" style="margin-top:10px;">跳过，暂时不填写</a> 
            </div>
        </div> 
