...
class MyViewAction extends ViewAction
{
    ...
    public function run()
    {
        ...
        if(in_array($this->controller->getRoute(),Yii::$app->params['cms_route'])){
    ...
    protected function resolveViewContent()
    {
        ...
        if($this->controller->className() != 'mobile\controllers\SandboxController'){
            $key = $this->controller->className() . '/' . $viewName;
            $obj = Yii::$app->cache->get($key);
            
            if($obj === false){
                $obj = \backend\models\Release::findOne(['view'=>$viewName,'platform' => 2]);
            ...
            $obj = \backend\models\Cms::find()->where("view = '".$viewName."' and status != -1 and platform = 2")->one();
        }
    ...
    protected function resolveViewHistoriesContent(){
        ...
        $obj = \backend\models\CmsHistory::findOne(['id'=>$viewName,'platform' => 2]);
    ...
    protected function getWidget($item){
        ...
        $str = '\mobile\widgets\\'.$widgetInfo[0];
