...
class CmsController extends Controller
{
    ...
    public function actionIndex($pagesize = 10)
    {
        ...
        $userId = 'admin';
        $storeIdList = [0=>'ftzmall',1=>'test']; //get storesid list by userid
        
        $storeId = array_keys($storeIdList)[0];
        if(isset($_COOKIE['storeId'])){
            $storeId = $_COOKIE['storeId'];
        }
        
        $qp = Yii::$app->request->queryParams;
        if(!isset($qp['CmsSearch'])){
            $qp['CmsSearch'] =  array();
            $qp['CmsSearch']['storeid'] = $storeId;
        }  else if(!isset($qp['CmsSearch']['storeid'])){
            $qp['CmsSearch']['storeid'] = $storeId;
        }else{
            $storeId = $qp['CmsSearch']['storeid'];
        }
         ...
         */
        $filterSelector = "#DataTables_Table_0_filter input, #DataTables_Table_0_length select, #storeid_dropdowmlist";
        $options = [
        ...
        setcookie('storeId',$storeId , time() + 3600 * 24 * 30, '/');
