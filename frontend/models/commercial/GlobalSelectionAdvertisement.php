 ...
 * 全球精选广告
 */
class GlobalSelectionAdvertisement extends BaseAdvertisement {
    ...
    public function __construct() {
        parent::__construct();
        $this->view = 'globalSelection';
        $this->type = 'globalSelection';
        $this->itemClass = '\frontend\models\commercial\GlobalSelectionItem';
    }
    ...
    protected function analyData($data){
        ...
        if(!empty($data['items'])){
            ...
            foreach($items as $item){
                ...
                Url::base().'/themes/ftzmallnew/src/images/baoshui.png';
                ...
                //后台未提供此类信息
                $newItem['newPro'] = is_array($item->tagName) &&  in_array('新品' , $item->tagName ) ? 1 : 0;
                $newItem['baoyou'] = is_array($item->tagName) &&  in_array('包邮' , $item->tagName ) ? 1 : 0;
                $newItem['emergency'] = 0;
                $newItem['soldout'] = !empty($item->soldout) ?  $item->soldout : 0;
