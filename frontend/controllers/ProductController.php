...
//                ],
    ...
    public function actionView($id = '') {
        ...
        $userId = Yii::$app->user->getId(); //获取用户默认地址    
        if (!$userId) {
            ...
            foreach ($parentCatgroups as $k => $v) {
                if ($v['catalogId'] == 'master') {
                    ...
                    foreach ($categoryRecommend as $k => $v) {
                        $categoryRecommend[$k]['fullImage'] = Tools::qnImg($v['desc']['fullImage'], 58, 58); //好像yii没有类似的arrayhelper函数，手动写了，小图的处理方式待定
                        if (!empty($v['parentCatentries'])) {
                            $categoryRecommend[$k]['id'] = $v['parentCatentries'][0]['id']; //对于子sku商品，在推荐页的链接应该跳转到product上
                        }
                        if (isset($v['promotionPriceInfo'][0]['price']) && $v['promotionPriceInfo'][0]['price'] != "") {
                            $categoryRecommend[$k]['offerPriceInfo'][0]['price'] = $v['promotionPriceInfo'][0]['price'];
                        }
                    }
                }
            }
        ...
        foreach ($tag as $k => $arr) {
            ...
            if (isset($arr['type']) && isset($arr['display']) && $arr['type'] == "NM" && $arr['display']) {
                $cartModel = new \frontend\models\CartApi($userId);
        ...
        if ($data['brand']['name'] && $data['brand']['name'] != "") {
            ...
            foreach ($brandRecommend as $k => $v) {
                $brandRecommend[$k]['fullImage'] = Tools::qnImg($v['desc']['fullImage'], 74, 74); //好像yii没有类似的arrayhelper函数，手动写了，小图的处理方式待定    
                if (!empty($v['parentCatentries'])) {
        ...
        $packageRecommend = array();
        if ($data["type"] == "package") {
            $packageRecommend = $this->getRecommendBypackage(11, "ftzmall", $id);
            foreach ($packageRecommend as $k => $v) {
                if (isset($v['promotionPriceInfo'][0]['price']) && $v['promotionPriceInfo'][0]['price'] != "") {
                    $packageRecommend[$k]['offerPriceInfo'][0]['price'] = $v['promotionPriceInfo'][0]['price'];
                }
                $packageRecommend[$k]['fullImage'] = Tools::qnImg($v['desc']['fullImage'], 74, 74); //好像yii没有类似的arrayhelper函数，手动写了，小图的处理方式待定
            }
        }
    ...
    public function getRecommendBypackage($num = 8, $memberId = "ftzmall", $id) {
        //Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $RecommendModel = new RecommendApi();
        $params = [
            "number" => $num, //optional, 推荐商品的数量,默认为8
        ...
        $recommend = $RecommendModel->recommendPackage($params);
        $key = key($recommend);
        $recommend = isset($recommend[$key]['result']) ? $recommend[$key]['result'] : [];
        $recommendTrue = array();
        foreach ($recommend as $k => $arr) {
