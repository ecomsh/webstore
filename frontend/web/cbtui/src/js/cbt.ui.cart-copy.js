$(function(){
	//优惠活动浮框js
	var activitiesCount=$(".order-activities");
	//alert(activitiesCount);
	for(var i=0;i<activitiesCount.length;i++){
		var arrowCount=$(activitiesCount[i]).children(".order-activities-arrow");
		if(arrowCount.length>1){
			//alert(arrowCount.length);
			//alert(arrowCount.text());
			arrowCount.css("display","none");
			arrowCount.siblings(".activities-popover").css("display","block");
			$(".activities-popover").popover({
				trigger:'hover',
				html:true
			})
		}
