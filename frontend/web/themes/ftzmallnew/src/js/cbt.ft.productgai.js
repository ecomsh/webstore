...
function changeProductprice(itempartNumber)
{   
        ...
        if(k == fatheritempartNumber) //先显示父类价格，再做子类覆盖
        {
            ...
            try{
                ...
                {                             
                    ...
                    {
                        if(pricelist[k]['offer'].CNY>800 || taxRate >50 || taxRate==50 )
                        {                   
            ...
            try{
                ...
                {   
                    if( salestype !=1 && salestype!=2)
                    {
                        if(pricelist[k]['promotion'].CNY>800 || taxRate >50 || taxRate==50 )
                        {                   
                            $('#addtocart').css('display','none');
                            $('#buynow').css('display','block');                 
                        }
        ...
        {  
            ...
            try{
                if (fatherpricelist['promotion'].CNY)
                {
                    if( salestype !=1 && salestype!=2)
                    {
                        if(fatherpricelist['promotion'].CNY>800 || taxRate >50 || taxRate==50 )
                        {                   
                            $('#addtocart').css('display','none');
                            $('#buynow').css('display','block');                 
                        }
...
function addtocart(_csrf, userId, loginurl, tmp_url){
    ...
    // var ceshi = document.getElementById("car-count"); 
    ...
    //         end: {
    //                 left: offset.left,
    //                 top: ceshi.getBoundingClientRect().top,
    //                 width: 0,
    //                 height: 0
    //         },
    //         onEnd: function(){
...
function getPrealtime() {
    var skuMapValue = $("input[name='skuMapValue']").val(); //productid数量什么的也这样读
    var skuMapId = $("input[name='skuMapId']").val();
    var params = ({
        "itemId": skuMapId,
        "fatheritempartNumber": fatheritempartNumber, //永远获取父类的itempartNumber
        "itempartNumber": skuMapValue,
        "shop": shop,
        "type": goodstype,
        "_csrf": _csrf,   
    ...
    $.ajax({
        type: 'post',
        //url: 'index.php?r=productgai/getprealtime',
        url: '../productgai/getprealtime.html',
        data: params,
        success: function(res) {
            changePrealtime(res['product']);
            ...
            changeProductprice(skuMapValue);
            /* 防止pricelist传回来为空或者错误数据的情况下税率不对，所以先基于search返回的数据做一次getTax */
